...
class MultiOrderDWConv(nn.Module): ## non-split
    """Multi-order Features with Dilated DWConv Kernel in MogaNet."""

    def __init__(self, embed_dims, dw_dilation=[1, 2, 3]):
        ...
        self.DW_conv1 = nn.Conv2d(
            in_channels=self.embed_dims, out_channels=self.embed_dims, kernel_size=5,
            padding='same',
            groups=self.embed_dims, stride=1, dilation=dw_dilation[1],
        )
        # DW conv 2
        self.DW_conv2 = nn.Conv2d(
            in_channels=self.embed_dims, out_channels=self.embed_dims, kernel_size=7,
            padding='same',
            groups=self.embed_dims, stride=1, dilation=dw_dilation[2],
        )
        # a channel convolution
        self.PW_conv = nn.Conv2d(
            in_channels=embed_dims*3, out_channels=embed_dims, kernel_size=1)

    def forward(self, x):
        B, C, H, W = x.shape
        x_0 = self.DW_conv0(x)
        x_1 = self.DW_conv1(x_0)
        x_2 = self.DW_conv2(x_0)
...
class MultiOrderGatedAggregation(nn.Module):
    ...
    def __init__(self, embed_dims, attn_dw_dilation=[1, 2, 3], attn_channel_split=[1, 3, 4], attn_shortcut=True):
        ...
        self.value = MultiOrderDWConv(
            embed_dims=embed_dims, dw_dilation=attn_dw_dilation)
